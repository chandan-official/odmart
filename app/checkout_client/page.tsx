...
export default function CheckoutPage() {
  ...
  const handleAddAddress = async () => {
    ...
    try {
      ...
      setNewAddress(initialAddress); setShowModal(false); fetchAddresses();
    } catch (err) { console.error("Add address failed:", err); alert("Failed to add address. Try again."); }
  }
  ...
  const handleCheckout = async () => {
    ...
    } catch (err) { console.error("Checkout failed:", err); alert("Checkout failed, please try again!"); }
  }
  ...
  return (
    <div className="co-container">
      ...
      <div className="co-section">
        ...
        <div className="co-price-row"><span>Subtotal</span><span>₹{cart.reduce((acc, item) => acc + item.price * item.qty, 0)}</span></div>
      </div>
      {/* Addresses */}
      <div className="co-section">
        ...
        {addresses.map(addr => (
          <label key={addr._id} className="co-address-option">
            ...
            <div><p className="co-address-title">{addr.label}</p><p className="co-address-text">{addr.text}</p></div>
          </label>
        ))}
        <button className="co-add-address-btn" onClick={() => setShowModal(true)}>+ Add New Address</button>
      </div>
      {/* Place Order */}
      <button className="co-place-btn" onClick={handleCheckout}>Place Order</button>
      {/* Modal */}
      {showModal && (
        <div className="co-modal">
          <div className="co-modal-box">
            ...
            <div className="co-modal-actions">
              <button className="co-modal-cancel" onClick={() => setShowModal(false)}>Cancel</button>
              <button className="co-modal-save" onClick={handleAddAddress}>Save Address</button>
            </div>
          </div>
        </div>
      )}
